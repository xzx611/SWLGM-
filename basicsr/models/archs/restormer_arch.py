...
#from GPU_memory import GPUMemoryProfiler
NEG_INF = -1000000
...
class MambaU(nn.Module):
    def __init__(self,
        ...
        super(MambaU, self).__init__()
        h = resolution[0]
        w = resolution[1]

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.localmamba1 = WindowmambaBlock(dim=dim,h=h, w=w, window_size=32, shift_size=3)
        self.localmamba2 = WindowmambaBlock(dim=dim * 2, h=h//2, w=w//2, window_size=16, shift_size=3)
        self.localmamba3 = WindowmambaBlock(dim=dim * 4, h=h//4, w=w//4,  window_size=8, shift_size=2)
        self.localmamba4 = WindowmambaBlock(dim=dim * 8, h=h//8, w=w//8,  window_size=4, shift_size=1)


        self.encoder_level1 = nn.Sequential(*[
            VSSBlock(hidden_dim=dim, h=h, w=w, d_state=d_state, expand=expand, d_conv=d_conv,drop_path=drop_path, attn_drop_rate=attn_drop_rate) for i in range(num_blocks[0])])

        self.down1_2 = Downsample(dim)  ## From Level 1 to Level 2 , init_value, heads_range, value_factor=1
        self.encoder_level2 = nn.Sequential(*[
            VSSBlock(hidden_dim=dim * 2, h=h//2, w=w//2, d_state=d_state, expand=expand, d_conv=d_conv,drop_path=drop_path, attn_drop_rate=attn_drop_rate) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[
            VSSBlock(hidden_dim=int(dim * 2 ** 2), h=h//4, w=w//4, d_state=d_state, expand=expand, d_conv=d_conv,drop_path=drop_path, attn_drop_rate=attn_drop_rate) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.latent = nn.Sequential(*[
            VSSBlock(hidden_dim=int(dim * 2 ** 3), h=h//8, w=w//8, d_state=d_state, expand=expand, d_conv=d_conv,drop_path=drop_path, attn_drop_rate=attn_drop_rate) for i in range(num_blocks[3])])
        ...
        self.decoder_level2 = nn.Sequential(*[
            VSSBlock(hidden_dim=int(dim * 2 ** 1), h=h//2, w=w//2, d_state=d_state, expand=expand, d_conv=d_conv,drop_path=drop_path, attn_drop_rate=attn_drop_rate) for i in range(num_blocks[1])])
    ...
    def forward(self, inp_img):
        ...
        #local_features = self.c(inp_enc_level1)  ##local
        #local_features = self.channeldown(local_features)##
        inp_enc_level1 =self.localmamba1(inp_enc_level1)
        #out_enc_level1 = inp_enc_level1
        out_enc_level1 = self.encoder_level1(inp_enc_level1) #+ self.localmamba1(inp_enc_level1)
        #print(out_enc_level1.shape)

        inp_enc_level2 = self.down1_2(out_enc_level1)
        inp_enc_level2 = self.localmamba2(inp_enc_level2)
        #out_enc_level2 = inp_enc_level2
        out_enc_level2 = self.encoder_level2(inp_enc_level2) #+ self.localmamba2(inp_enc_level2)
        #print(out_enc_level2.shape)

        inp_enc_level3 = self.down2_3(out_enc_level2)
        inp_enc_level3 = self.localmamba3(inp_enc_level3)
        #out_enc_level3 = inp_enc_level3
        out_enc_level3 = self.encoder_level3(inp_enc_level3)  # + self.localmamba3(inp_enc_level3)
        #print(out_enc_level3.shape)

        inp_enc_level4 = self.down3_4(out_enc_level3)
        inp_enc_level4 = self.localmamba4(inp_enc_level4)
        #latent = inp_enc_level4
        latent = self.latent(inp_enc_level4)  # + self.localmamba4(inp_enc_level4)
        #print(latent.shape)

        inp_dec_level3 = self.up4_3(latent)
        inp_dec_level3 = cat(inp_dec_level3, out_enc_level3)
        inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
        inp_dec_level3 = self.localmamba3(inp_dec_level3)
        #out_dec_level3 = inp_dec_level3
        out_dec_level3 = self.decoder_level3(inp_dec_level3)  # + self.localmamba3(inp_dec_level3)
        #print(out_dec_level3.shape)

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = cat(inp_dec_level2, out_enc_level2)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        inp_dec_level2 = self.localmamba2(inp_dec_level2)
        #out_dec_level2 = inp_dec_level2
        out_dec_level2 = self.decoder_level2(inp_dec_level2)  # + self.localmamba2(inp_dec_level2)
        #print(out_dec_level2.shape)

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = cat(inp_dec_level1, out_enc_level1)
        inp_dec_level1 = self.reduce_chan_level1(inp_dec_level1)
        inp_dec_level1 = self.localmamba1(inp_dec_level1)
        #out_dec_level1 = inp_dec_level1
        out_dec_level1 = self.decoder_level1(inp_dec_level1)  # + self.localmamba1(inp_dec_level1)
        #print(out_dec_level1.shape)

        out = self.output(out_dec_level1) + inp_img #+local_features




        return out_dec_level1, out


if __name__ == '__main__':
    device = torch.device('cuda')
    data = torch.randn([1, 3, 256, 256]).to(device)
    model = MambaU().to(device)
    flops,params= profile(model,(data,))
    print('flops(G)', flops/1000000000.0, 'params(M)',params/1000000.0)
    #print("model:", sum(p.numel() for p in model.parameters() if p.requires_grad))
    out = model(data)
    print(torch.cuda.max_memory_allocated()/(1024**2))
    print('restormer_out', out.shape)
    #profiler = GPUMemoryProfiler()
    #profiler.profile_model(model, (3, 256, 256), batch_sizes=[1, 4, 8, 16])
